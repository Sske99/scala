...
object Compositions {
  ...
  def main(args: Array[String]): Unit = {
    val x: Int=3
    println("a) " + Compositions.testCompose((x:Int) => x+x)((x:Int)=> x*2)((x:Int)=> x-2)(x))
    println("b) " + Compositions.testMapFlatMap((x:Int)=>Some(x+x))((x:Int)=>Some(x*2))((x:Int)=>x-2)(Some(x)))
    println("c) " + Compositions.testForComprehension((x:Int)=>Some(x+x))((x:Int)=>Some(x*2))((x:Int)=>x-2)(Some(x)))
