...
object RecursiveFunc {
  ...
  def main(args: Array[String]): Unit = {
    ...
    println("a) "+RecursiveFunc.testReverse(list1))
    println("b) "+RecursiveFunc.testMap(list1, (x:Int) => x+1))
    println("c) "+RecursiveFunc.testAppend(list1,list2))
    println("d) "+RecursiveFunc.testFlatMap(list1,(x:Int)=> Cons(x*2,Nil())))
  }
}
