...
object PatternMatching {
  ...
  def main(args: Array[String]): Unit ={
    val dog = Mammal("dog", Meat, 20)
    val raven = Bird("raven", Vegetables)

    println("a) " + testIntToString(3))
    println("b) " + testIsMaxAndMoritz("Max"))
    println("c) " + testIsEven(10))
    println("d) " + testWinsA(Paper, Paper))
    println("e) " + testExtractMammalWeight(dog))
    println("f) " + testUpdateFood(raven).food)
  }
}
