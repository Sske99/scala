...
def listHead(list:List[Int]):Int = list match {
  ...
  def main(args: Array[String]): Unit = {
    println("a) " + RecursiveData.listEmpty(Nil()))
    println(RecursiveData.listEmpty(Cons(1,Cons(2,Cons(3,Nil())))))
    println("b) " + RecursiveData.listHead(Cons(1,Cons(2,Cons(3,Nil())))))
    println(RecursiveData.listHead(Nil()))
